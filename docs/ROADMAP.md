...
- Bridgehead candidate: `?bridgehead` now implements one complete full-layout mission loop while preserving `?full-level` as the art/LOD survey. Ordered entry/exit gates make the selected bridge a required traversal before the Warden encounter. The packaged smoke passes 38 assertions, including invalid pre-entry/unboosted crossings, both win routes, the real Warden GLB's idle/attack/run/death states, a 1.2 s disarmed recovery, result mastery telemetry, one rendered loss, duplicate-terminal rejection and ten scripted loss/retry cycles. Win resets settle at 85 geometries/53 textures and nine additional loss resets remain flat at 86 geometries; 46 tests/typecheck/build pass. Adversarial briefs and the >=90/100 gate are in `BRIDGEHEAD_RUN_SCORECARD.md`, `ART_DIRECTION_CRITICAL_BRIEF.md` and `PLAYER_ADVOCATE_AUDIT.md`.
- The latest embedded exact-render sample is 112 combat frames at 1280×720 renderer pixels: 30.8 ms median / 33.0 ms p95. It fails the ≤20 ms p95 gate and does not replace three declared foreground hardware runs. Profile before reducing visible quality blindly.
- Immediate Bridgehead queue: (1) Blender-author a 4 m-clear south/north airborne lane and measured court floor without overwriting sources, (2) export/version markers and <=35 resident proxies, (3) capture launch/bridge/court/extraction at fixed1280x720, (4) natural pointer-lock and drag traversal of both routes, (5) ten **natural full-sortie** retry cycles and foreground p95 (the scripted lifecycle cycles are not substitutes), (6) fresh-player gate. Do not add more enemies, weapons or missions before this candidate reaches the scorecard gate.

- Full-level design pass: `docs/FULL_LEVEL_GAME_DESIGN.md` defines the Bridgehead Run route using the user's screenshot and new generated boards in `art/concepts/`. Player readability was corrected with a length-aware AVI jetbike scale contract. Next step is an authored collision/marker package for the selected route, not freeform whole-level combat.
...
- Immediate queue: (1) human foreground mouse/route playtest, (2) visible mesh/collision boundary tuning, (3) visual review and timing polish of the implemented Warden attack/death choreography, (4) material and measured performance polish, (5) provenance and external playtest gates. Automated success does not close premium validation.
