...
Start with [RED_HORIZON_VISUAL_DIRECTOR_REVIEW.md](RED_HORIZON_VISUAL_DIRECTOR_REVIEW.md), [RED_HORIZON_REAL_MODEL_PATH_PLAN.md](RED_HORIZON_REAL_MODEL_PATH_PLAN.md) and `art/real-path/red-horizon-real-path-layout.json`. The feature inspection is `art/real-path/real-level-feature-report.json`; it was generated from `public/models/level1.glb` in Blender and uses the same normalized full-level coordinates as `scripts/build-full-level.py`. The rendered local design set is under ignored `art/real-path/*.png`, with the contact sheet at `art/real-path/red-horizon-real-model-path-contact.png` and the editable local Blender copy at `art/real-path/red-horizon-real-model-path.blend`. Remote-viewable JPGs are tracked under `docs/images/`.

Critical scale/path decision: the first real-model pass was not good enough. The Warden data was not actually giant, but the shot staged it too loudly; the contract now treats it as a 2.05 m elite rival. The path no longer presents one continuous floating debug spline in close-up views. The script projects route decals onto actual surfaces where raycasts are close enough, and uses vertical air gates for deliberate chasm/drop segments.
