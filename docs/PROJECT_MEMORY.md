...
**September 17 physical iPad result:** the current iPad touch-control model failed the feel gate. The user reported that camera movement and gameplay were really bad, barely worked, and did not move properly. Do not keep tuning this as if it is one sensitivity bug. Start the mobile rethink from [IPAD_CONTROL_RETHINK.md](IPAD_CONTROL_RETHINK.md): preserve the useful iPad-stage plumbing, but replace the dual-stick desktop `fly-controls` mapping with a separate mobile-first assisted route/camera model.
...
Start with [RED_HORIZON_REAL_MODEL_PATH_PLAN.md](RED_HORIZON_REAL_MODEL_PATH_PLAN.md), [RED_HORIZON_VISUAL_DIRECTOR_REVIEW.md](RED_HORIZON_VISUAL_DIRECTOR_REVIEW.md) and `art/real-path/red-horizon-real-path-layout.json`. The feature inspection is `art/real-path/real-level-feature-report.json`; it was generated from `public/models/level1.glb` in Blender and uses the same normalized full-level coordinates as `scripts/build-full-level.py`. The rendered local design set is under ignored `art/real-path/*.png`, with the contact sheet at `art/real-path/red-horizon-real-model-path-contact.png` and the editable local Blender copy at `art/real-path/red-horizon-real-model-path.blend`. Remote-viewable JPGs are tracked under `docs/images/`.

Critical design decision: the planetary cannon is a background landmark only. The playable start moved to the real left bridgehead because the cannon route forced the path to float above the model. The latest grounding receipts show all high, low and extraction route points as surface decals with 0.45 m hover height and no `air-gate-needed` points. The Warden stays as the supplied model at elite-rival scale, about 2.05 m for the visual pass.
