...
const fullMode = !surveyMode && (modeParams.has('bridgehead') || (!modeParams.has('ridge-run') && !modeParams.has('arena') && !modeParams.has('playtest')));
const streamMode = fullMode || surveyMode;
const failedArtFixture = fullMode && modeParams.has('playtest') && modeParams.has('asset-failure');
const ridgeMode = !streamMode && modeParams.has('ridge-run');
...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    document.getElementById('player')?.setAttribute('fly-controls', `lookSensitivity: ${sensitivity}; invertY: ${invertY}; movementSpeed: ${fullMode ? BRIDGEHEAD_SPEED : 25}; cameraHeight: ${fullMode ? BRIDGEHEAD_CAMERA.height : 2}; cameraDistance: ${fullMode ? BRIDGEHEAD_CAMERA.distance : 8}; cameraShoulder: ${fullMode ? BRIDGEHEAD_CAMERA.shoulder : 0}`);
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      if (fullMode && pos && projectionCamera && objectiveTargets.current.length) {
        ...
        setTargetReadouts(objectiveTargets.current.map(item => {
          ...
          left=Math.max(8,Math.min(92,left));top=Math.max(12,Math.min(86,top));
  ...
  const capture = () => {
    ...
    player?.setAttribute('fly-controls', `enabled: true; dragToLook: false; lookSensitivity: ${sensitivity}; invertY: ${invertY}; movementSpeed: ${fullMode ? BRIDGEHEAD_SPEED : 25}; cameraHeight: ${fullMode ? BRIDGEHEAD_CAMERA.height : 2}; cameraDistance: ${fullMode ? BRIDGEHEAD_CAMERA.distance : 8}; cameraShoulder: ${fullMode ? BRIDGEHEAD_CAMERA.shoulder : 0}`);
    try {
  ...
  const cursorFlight = () => {
    ...
    gameAudio.resume();
    gameAudio.startAmbient();
    document.getElementById('player')?.setAttribute('fly-controls', `enabled: true; dragToLook: true; lookSensitivity: ${sensitivity}; invertY: ${invertY}; movementSpeed: ${fullMode ? BRIDGEHEAD_SPEED : 25}; cameraHeight: ${fullMode ? BRIDGEHEAD_CAMERA.height : 2}; cameraDistance: ${fullMode ? BRIDGEHEAD_CAMERA.distance : 8}; cameraShoulder: ${fullMode ? BRIDGEHEAD_CAMERA.shoulder : 0}`);
  ...
  const arena = useMemo(() => (
    <a-scene world-stream={streamMode ? (fullMode ? (failedArtFixture ? `playable: true; scale: ${BRIDGEHEAD_ENVIRONMENT_SCALE}; manifest: models/level1-stream/missing.json` : `playable: true; scale: ${BRIDGEHEAD_ENVIRONMENT_SCALE}`) : '') : undefined} level-runtime={ridgeMode ? '' : undefined} ridge-run={ridgeMode ? '' : undefined} bridgehead-run={fullMode ? '' : undefined} data-playtest={new URLSearchParams(window.location.search).has('playtest') ? 'true' : undefined} ref={sceneRef} game-manager="enemyCount: 2; level: 1; spawnRadius: 16; maxActiveEnemies: 4; enemySpawnInterval: 1200" gltf-model="dracoDecoderPath: vendor/draco/" vr-mode-ui="enabled: false" renderer="maxCanvasWidth: 1280; maxCanvasHeight: 720; antialias: false; colorManagement: true; toneMapping: ACESFilmic; exposure: 1.15; precision: high" background="color: #130b14" fog={streamMode ? 'type: exponential; color: #36303c; density: 0.002' : 'type: exponential; color: #a06951; density: 0.009'}>
      ...
      <a-entity id="player" position="0 3 12" player-component="" fly-controls={fullMode ? `enabled: false; lookSensitivity: 0.5; movementSpeed: ${BRIDGEHEAD_SPEED}; cameraHeight: ${BRIDGEHEAD_CAMERA.height}; cameraDistance: ${BRIDGEHEAD_CAMERA.distance}; cameraShoulder: ${BRIDGEHEAD_CAMERA.shoulder}` : 'enabled: false; lookSensitivity: 0.5'}>
        <a-entity id="camera-rig" position={fullMode ? `${BRIDGEHEAD_CAMERA.shoulder} ${BRIDGEHEAD_CAMERA.height} ${BRIDGEHEAD_CAMERA.distance}` : '0 2 8'}><a-camera id="camera" near={fullMode ? 0.1 : 0.005} far={fullMode ? 500 : 10000} position="0 0 0" look-controls="enabled: false" wasd-controls="enabled: false" /></a-entity>
  ...
  return <div className={`App ${paused ? 'is-paused' : ''} ${reducedMotion ? 'reduced-motion' : ''}`}>
    <div className="hud" style={{ visibility: started && !result ? 'visible' : 'hidden' }}>
      ...
      <div id="score-ui"><small>OPERATION / RED HORIZON</small><div>{fullMode ? 'BRIDGEHEAD RUN' : surveyMode ? 'FULL LEVEL · SURVEY' : ridgeMode ? 'RIDGE RUN' : <>WAVE <span id="level-value">1</span> / 3</>}</div><div>SCORE <span id="score-value">0</span></div><div>HOSTILES <span id="enemies-value">0</span></div><div id="combo-value">CHAIN ×1</div></div>
      ...
      {(ridgeMode || fullMode) && <div className="ridge-objective"><strong>{objective}</strong><small>{routeBonus}</small></div>}
      {fullMode && <div className="objective-targets">{targetReadouts.map(item => <div key={item.label} className={`objective-target ${item.offscreen ? 'offscreen' : ''}`} style={{left:`${item.left}%`,top:`${item.top}%`}}><b>{item.offscreen ? '◆ ' : ''}{item.label}</b><span>{Math.round(item.distance)}m · {item.altitude>=0?'+':''}{Math.round(item.altitude)}m ALT{item.approach ? ` · ${item.approach}` : ''}</span>{item.label==='EXTRACTION' && <i><em style={{width:`${Math.round(objectiveProgress*100)}%`}} /></i>}</div>)}</div>}
      ...
      <div className="controls-hint">WASD fly · E / Q altitude · Shift boost · Captured: move mouse · Drag: right-look / left-aim+fire · Esc pause</div>
    ...
    {(!started || paused || result) && <div className="menu-shade"><main className="mission-menu">
      ...
      <p className="menu-description">{result ? `Score ${result.score} · Best ${result.best} · ${(ridgeMode || fullMode) ? `${result.route || 'No'} route · ${result.seconds}s · ${result.shots || 0} shots · ${result.hullLost || 0} hull lost · ${result.chargesSpent || 0} charges spent` : `Wave ${result.level}/3`}` : fullMode ? 'Choose charged fire or a shield. Cross the chasm, bait the enemy aim lock, then strike while its armor is open. Reach the extraction platform.' : surveyMode ? 'Explore the complete original level layout. Coarse terrain stays visible while nearby high-detail regions stream on demand. This editor survey has no collision or combat.' : ridgeMode ? 'A short route experiment. Boost through the high cyan gate for three double-damage shots, or take the low amber gate for 30 shield. Defeat the Warden, then hold inside the gold extraction ring.' : 'Pilot a combat jetbike over the red frontier. Clear three waves. Chain eliminations within six seconds to multiply your score.'}</p>
      <div className="mission-details"><div><small>MISSION</small><strong>{fullMode ? 'Bridgehead Run' : surveyMode ? 'Full-level survey' : ridgeMode ? 'Ridge Run' : 'Three-wave sortie'}</strong></div><div><small>LOADOUT</small><strong>Twin pulse cannon</strong></div><div><small>FLIGHT</small><strong>Mouse + keyboard</strong></div></div>
      ...
      <p className="menu-controls">WASD move · ← → steer &nbsp; / &nbsp; E rise · Q descend &nbsp; / &nbsp; Shift boost<br />Captured mouse: move to aim · Drag mode: hold right to aim or left to aim + fire<br />R reload &nbsp; / &nbsp; Esc pause</p>
    </main></div>}
